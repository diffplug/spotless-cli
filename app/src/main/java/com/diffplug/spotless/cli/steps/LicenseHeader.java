...
public class LicenseHeader extends SpotlessFormatterStep {
    ...
    @CommandLine.Option(
            ...
            required = false,
            description =
                    "The delimiter to use for the license header. If not provided, the delimiter will be guessed based on the first few files we find. Otherwise, 'java' will be assumed.")
    String delimiter;
    ...
    static class LicenseHeaderSourceOption {
        @CommandLine.Option(
                names = {"--header", "-H"},
                required = true,
                description = "The license header content to apply. May contain @|YELLOW $YEAR|@ as placeholder.")
        String header;
        ...
        @CommandLine.Option(
                ...
                required = true,
                description =
                        "The license header content in a file to apply.%n  May contain @|YELLOW $YEAR|@ as placeholder.")
        File headerFile;
    }
    ...
    @CommandLine.Option(
            ...
            description = "How and if the year in the copyright header should be updated."
    ...
    @CommandLine.Option(
            names = {"--skip-lines-matching", "-s"},
            required = false,
            description = "Skip lines matching the given regex pattern before inserting the licence header.")
    String skipLinesMatching;

    @CommandLine.Option(
            names = {"--content-pattern", "-c"},
            required = false,
            description =
                    "The pattern to match the content of the file before inserting the licence header. (If the file content does not match the pattern, the header will not be inserted/updated.)")
    String contentPattern;

    @NotNull @Override
    public List<FormatterStep> prepareFormatterSteps(SpotlessActionContext context) {
        FormatterStep licenseHeaderStep = LicenseHeaderStep.headerDelimiter(
                        headerSource(context), delimiter(context.targetFileType()))
    ...
    private ThrowingEx.Supplier<String> headerSource(SpotlessActionContext context) {
        if (licenseHeaderSourceOption.header != null) {
            ...
            return () -> ThrowingEx.get(() -> Files.readString(
                    context.resolveFile(licenseHeaderSourceOption.headerFile).toPath()));
        }
    }
